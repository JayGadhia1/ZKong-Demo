...
class SupabaseService:
    ...
    def get_store_mapping(
        ...
        """
        try:
            result = self.client.table("store_mappings").select("*").eq(
                "source_system", source_system
            ).eq("source_store_id", source_store_id).eq(
                "is_active", True
            ).single().execute()
    ...
    def create_store_mapping(self, mapping: StoreMapping) -> StoreMapping:
        ...
        try:
            result = self.client.table("store_mappings").insert(
                mapping.dict(exclude_none=True, exclude={"id"})
            ).execute()
    ...
    def create_or_update_product(self, product: Product) -> Product:
        ...
        try:
            ...
            if existing:
                ...
                result = self.client.table("products").update(update_data).eq(
                    "id", existing.id
                ).execute()
                ...
                result = self.client.table("products").insert(
                    product.dict(exclude_none=True, exclude={"id"})
                ).execute()
    ...
    def get_product_by_source(
        ...
        try:
            query = self.client.table("products").select("*").eq(
                "source_system", source_system
    ...
    def get_pending_sync_queue_items(self, limit: int = 10) -> List[SyncQueueItem]:
        ...
        """
        try:
            result = self.client.table("sync_queue").select("*").eq(
                "status", "pending"
    ...
    def update_sync_queue_status(
        ...
        try:
            ...
            result = self.client.table("sync_queue").update(update_data).eq(
    ...
    def create_or_update_zkong_product(self, zkong_product: ZKongProduct) -> ZKongProduct:
        """Create or update ZKong product mapping."""
        try:
            # Check if mapping exists
            result = self.client.table("zkong_products").select("*").eq(
                "product_id", str(zkong_product.product_id)
            ).eq("store_mapping_id", str(zkong_product.store_mapping_id)).execute()
    ...
    def get_zkong_product_by_product_id(
        ...
        try:
            ...
            ).eq("store_mapping_id", str(store_mapping_id)).single().execute()
